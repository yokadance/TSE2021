...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "vacunas.uy_grupo16-ear.ear");

        // Current directory is the root of the "vacunas.uy_grupo16-web" project. Go up one level
        // and enter the "vacunas.uy_grupo16-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../vacunas.uy_grupo16-ejb/target/vacunas.uy_grupo16-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "vacunas.uy_grupo16-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/vacunas.uy_grupo16-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "vacunas.uy_grupo16-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
